...
import likeIcon from "../assets/images/web-push/web-push-icon_like.png";
import repostIcon from "../assets/images/web-push/web-push-icon_repost.png";
...
const locales = import.meta.compileTime<Record<string, Record<string, string>>>(
  "./web-push-locales.ts"
);
...
interface APINotification
  extends Omit<NotificationEntity, "account" | "status"> {
...
const formatMessage = (
  ...
  new IntlMessageFormat(locales[locale][messageId], locale).format(
    values
  ) as string;
...
const handlePush = (event: PushEvent) => {
  ...
  event.waitUntil(
    fetchFromApi(`/api/notifications/${notification_id}`, "get", access_token)
      .then((notification) => {
        const options: ExtendedNotificationOptions = {
          ...
          data: {
            ...
            url: notification.status
              ? `/@${notification.account.username}/posts/${notification.status.id}`
              : `/@${notification.account.username}`,
          },
        };
        ...
        if (
          ...
          options.actions = [
            actionRepost(preferred_locale),
            actionLike(preferred_locale),
          ];
        }
...
const actionRepost = (preferred_locale: string) => ({
  action: "repost",
  icon: repostIcon,
  title: formatMessage("status.repost", preferred_locale),
});

/** Native action to like status. */
const actionLike = (preferred_locale: string) => ({
  action: "like",
  icon: likeIcon,
  title: formatMessage("status.like", preferred_locale),
});
...
const expandNotification = (notification: Notification) => {
  ...
  newNotification.actions = [
    actionRepost(notification.data.preferred_locale),
    actionLike(notification.data.preferred_locale),
  ];
...
const removeActionFromNotification = (
  ...
  const reactToNotificationClick = new Promise((resolve, reject) => {
    if (event.action) {
      if (event.action === "expand") {
        ...
        resolve(
          fetchFromApi(
            `/api/statuses/${data.id}/repost`,
            "post",
            data.access_token
          ).then(() =>
            removeActionFromNotification(event.notification, "repost")
          )
        );
        ...
        resolve(
          fetchFromApi(
            `/api/statuses/${data.id}/like`,
            "post",
            data.access_token
          ).then(() => removeActionFromNotification(event.notification, "like"))
        );
      } else {
