...
const ChatPageMain = () => {
  ...
  const handleBlockUser = () => {
    dispatch(openModal('CONFIRM', {
      heading: intl.formatMessage(messages.blockHeading, { username: chat?.account.username }),
  ...
  const handleUnblockUser = () => {
    dispatch(openModal('CONFIRM', {
      heading: intl.formatMessage(messages.unblockHeading, { username: chat?.account.username }),
  ...
  if (!currentChatId && chats && chats.length > 0) {
    return <ChatPageBlankslateWithChats />;
  }

  if (!currentChatId) {
    return <ChatPageBlankslateEmpty />;
  }
            ...
            <Stack space={4} className='px-6 py-5'>
              <HStack alignItems='center' space={3}>
                ...
                <Stack>
                  ...
                  <Text size='sm' theme='primary'>@{chat.account.username}</Text>
                </Stack>
              </HStack>
