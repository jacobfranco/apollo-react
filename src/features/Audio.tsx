...
const Audio: React.FC<IAudio> = (props) => {
  ...
  return (
    <div
      role="menuitem"
      className={clsx(
        "relative box-border overflow-hidden rounded-[10px] bg-black pb-11",
    ...
    >
      ...
      >
        <div
          className="absolute top-0 block h-1 rounded-md bg-white/20"
          style={{ width: `${buffer}%` }}
        />
        ...
        <span
          className={clsx(
            "absolute -top-1 z-30 -ml-1.5 size-3 rounded-full bg-accent-500 opacity-0 shadow-[1px_2px_6px_rgba(0,0,0,0.3)] transition-opacity duration-100",
      ...
      <div
        className={clsx(
          "absolute inset-x-0 bottom-0 z-20 box-border bg-gradient-to-t from-black/70 to-transparent px-[10px] opacity-100 transition-opacity duration-100 ease-linear"
        )}
      >
        <div className="my-[-5px] flex justify-between pb-3.5">
          <div className="flex w-full flex-auto items-center truncate text-[16px]">
            <button
              ...
              className={clsx(
                "inline-block flex-none border-0 bg-transparent px-[6px] py-[5px] text-[16px] text-white/75 opacity-75 outline-none hover:text-white hover:opacity-100 active:text-white active:opacity-100 "
              )}
              onClick={togglePlay}
            >
              <SvgIcon
            ...
            <div
              className={clsx(
                ...
                { "overflow-visible w-[50px] mr-[16px]": hovered }
              )}
