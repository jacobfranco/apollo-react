...
const getNotifications = createSelector(
  [
    (state) =>
      getSettings(state).getIn(["notifications", "quickFilter", "show"]),
    (state) =>
      getSettings(state).getIn(["notifications", "quickFilter", "active"]),
    (state) =>
      ImmutableList(
        (
          getSettings(state).getIn(["notifications", "shows"]) as ImmutableMap<
            string,
            boolean
          >
        )
  ...
  (
    ...
    allowedType,
    excludedTypes,
    notifications: ImmutableList<NotificationEntity>
  ) => {
...
const Notifications = () => {
  ...
  const filterBarContainer = showFilterBar ? <NotificationFilterBar /> : null;
  ...
  const scrollContainer = (
    <ScrollableList
      ...
      listClassName={clsx({
        "divide-y divide-gray-200 dark:divide-primary-800 divide-solid":
