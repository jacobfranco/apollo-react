import { deleteEntities } from "src/entity-store/actions";
...
import type { GroupRole } from "src/reducers/group-memberships";
...
const GROUP_MEMBERSHIP_REQUESTS_FETCH_REQUEST =
  "GROUP_MEMBERSHIP_REQUESTS_FETCH_REQUEST";
const GROUP_MEMBERSHIP_REQUESTS_FETCH_SUCCESS =
  "GROUP_MEMBERSHIP_REQUESTS_FETCH_SUCCESS";
const GROUP_MEMBERSHIP_REQUESTS_FETCH_FAIL =
...
const GROUP_MEMBERSHIP_REQUESTS_EXPAND_SUCCESS =
  "GROUP_MEMBERSHIP_REQUESTS_EXPAND_SUCCESS";
const GROUP_MEMBERSHIP_REQUESTS_EXPAND_FAIL =
...
const GROUP_MEMBERSHIP_REQUEST_AUTHORIZE_SUCCESS =
...
const GROUP_MEMBERSHIP_REQUEST_REJECT_REQUEST =
  "GROUP_MEMBERSHIP_REQUEST_REJECT_REQUEST";
const GROUP_MEMBERSHIP_REQUEST_REJECT_SUCCESS =
  "GROUP_MEMBERSHIP_REQUEST_REJECT_SUCCESS";
const GROUP_MEMBERSHIP_REQUEST_REJECT_FAIL =
  ...
  () => (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    return api(getState)
      ...
      .then((data) => {
        ...
        dispatch(
          fetchGroupRelationships(data.map((item: APIEntity) => item.id))
        );
      })
...
const fetchGroupRelationships =
  ...
  (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    return api(getState)
      .get(
        `/api/groups/relationships?${newGroupIds
          .map((id) => `id[]=${id}`)
          .join("&")}`
      )
      .then((response) => response.json())
...
const groupKick =
  ...
  (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    return api(getState)
      .post(`/api/groups/${groupId}/kick`, { account_ids: [accountId] })
...
const expandGroupBlocksSuccess = (
  id: string,
  accounts: APIEntity[],
  next: string | null
) => ({
...
const groupBlock =
  ...
  (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    return api(getState)
      .post(`/api/groups/${groupId}/blocks`, { account_ids: [accountId] })
...
const groupUnblock =
  ...
  (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    return api(getState)
      .delete(`/api/groups/${groupId}/blocks?account_ids[]=${accountId}`)
...
const groupPromoteAccount =
  ...
  (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    return api(getState)
      ...
      .then((response) => response.json())
      .then((data) =>
        dispatch(groupPromoteAccountSuccess(groupId, accountId, data))
      )
      .catch((err) =>
        dispatch(groupPromoteAccountFail(groupId, accountId, err))
      );
  };
...
const groupPromoteAccountSuccess = (
  groupId: string,
  accountId: string,
  memberships: APIEntity[]
) => ({
...
const groupDemoteAccount =
  ...
  (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    return api(getState)
      ...
      .then((response) => response.json())
      .then((data) =>
        dispatch(groupDemoteAccountSuccess(groupId, accountId, data))
      )
      .catch((err) =>
        dispatch(groupDemoteAccountFail(groupId, accountId, err))
      );
  };
...
const groupDemoteAccountSuccess = (
  groupId: string,
  accountId: string,
  memberships: APIEntity[]
) => ({
...
const fetchGroupMemberships =
  ...
  (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    return api(getState)
      .get(`/api/groups/${id}/memberships`, { searchParams: { role } })
      .then(async (response) => {
        const next = response.next();
        const data = await response.json();
        ...
        dispatch(
          importFetchedAccounts(
            data.map((membership: APIEntity) => membership.account)
          )
        );
...
const fetchGroupMembershipsSuccess = (
  id: string,
  role: GroupRole,
  memberships: APIEntity[],
  next: string | null
...
const fetchGroupMembershipsFail = (
  id: string,
  role: GroupRole,
  error: unknown
) => ({
...
const expandGroupMembershipsSuccess = (
  id: string,
  role: GroupRole,
  memberships: APIEntity[],
  next: string | null
...
const expandGroupMembershipsFail = (
  id: string,
  role: GroupRole,
  error: unknown
) => ({
...
const fetchGroupMembershipRequests =
  (id: string) => (dispatch: AppDispatch, getState: () => RootState) => {
    dispatch(fetchGroupMembershipRequestsRequest(id));

    return api(getState)
      .get(`/api/groups/${id}/membership_requests`)
      .then(async (response) => {
        const next = response.next();
        const data = await response.json();
...
const fetchGroupMembershipRequestsSuccess = (
  id: string,
  accounts: APIEntity[],
  next: string | null
) => ({
...
const expandGroupMembershipRequests =
  (id: string) => (dispatch: AppDispatch, getState: () => RootState) => {
...
const expandGroupMembershipRequestsSuccess = (
  id: string,
  accounts: APIEntity[],
  next: string | null
) => ({
...
const authorizeGroupMembershipRequest =
  (groupId: string, accountId: string) =>
  (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    return api(getState)
      ...
      .then(() =>
        dispatch(authorizeGroupMembershipRequestSuccess(groupId, accountId))
      )
      .catch((error) =>
        dispatch(authorizeGroupMembershipRequestFail(groupId, accountId, error))
      );
  };

const authorizeGroupMembershipRequestRequest = (
  groupId: string,
  accountId: string
) => ({
...
const authorizeGroupMembershipRequestSuccess = (
  groupId: string,
  accountId: string
) => ({
...
const authorizeGroupMembershipRequestFail = (
  groupId: string,
  accountId: string,
  error: unknown
) => ({
...
const rejectGroupMembershipRequest =
  (groupId: string, accountId: string) =>
  (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    return api(getState)
      ...
      .then(() =>
        dispatch(rejectGroupMembershipRequestSuccess(groupId, accountId))
      )
      .catch((error) =>
        dispatch(rejectGroupMembershipRequestFail(groupId, accountId, error))
      );
  };

const rejectGroupMembershipRequestRequest = (
  groupId: string,
  accountId: string
) => ({
...
const rejectGroupMembershipRequestSuccess = (
  groupId: string,
  accountId: string
) => ({
...
const rejectGroupMembershipRequestFail = (
  groupId: string,
  accountId: string,
  error?: unknown
) => ({
