...
import type { Account, Group } from "src/schemas/index";
...
const COMPOSE_SET_GROUP_TIMELINE_VISIBLE =
  "COMPOSE_SET_GROUP_TIMELINE_VISIBLE" as const;
...
const COMPOSE_SUGGESTION_TAGS_UPDATE =
  "COMPOSE_SUGGESTION_TAGS_UPDATE" as const;

const COMPOSE_TAG_HISTORY_UPDATE = "COMPOSE_TAG_HISTORY_UPDATE" as const;

const COMPOSE_SUGGESTION_SPACES_UPDATE =
  "COMPOSE_SUGGESTION_SPACES_UPDATE" as const;
...
const messages = defineMessages({
  ...
  editSuccess: {
    id: "compose.edit_success",
    defaultMessage: "Your post was edited",
  },
  uploadErrorLimit: {
...
const replyCompose =
  (status: Status) => (dispatch: AppDispatch, getState: () => RootState) => {
...
const groupComposeModal =
  (group: Group) => (dispatch: AppDispatch, getState: () => RootState) => {
...
const mentionCompose = (account: Account) => (dispatch: AppDispatch) => {
...
const directCompose = (account: Account) => (dispatch: AppDispatch) => {
...
const submitCompose =
  (composeId: string, opts: SubmitComposeOpts = {}) =>
  async (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    return dispatch(createStatus(params, idempotencyKey, statusId))
      .then(function (data) {
        if (
          !statusId &&
          data.visibility === "direct" &&
          getState().conversations.mounted <= 0 &&
          history
        ) {
...
const changeUploadCompose =
  (composeId: string, id: string, params: Record<string, any>) =>
  (dispatch: AppDispatch, getState: () => RootState) => {
    if (!isLoggedIn(getState)) return;
...
const setGroupTimelineVisible = (
  composeId: string,
  groupTimelineVisible: boolean
) => ({
...
const fetchComposeSuggestionsEmojis = (
  dispatch: AppDispatch,
  composeId: string,
  token: string
) => {
...
const fetchComposeSuggestionsTags = (
  ...
  api(getState)
    ...
    .then((data) => {
      dispatch(
        updateSuggestionTags(composeId, token, data?.hashtags.map(normalizeTag))
      );
    })
    .catch((error) => {
      if (error instanceof HTTPError) {
...
const insertIntoSpaceHistory =
  (composeId: string, recognizedSpaces: APIEntity[], text: string) =>
  (dispatch: AppDispatch, getState: () => RootState) => {
    const state = getState();
    ...
    const intersectedOldHistory = oldHistory.filter(
      (id) =>
        ids.findIndex((newId) => newId.toLowerCase() === id.toLowerCase()) ===
        -1
    );

    ids.push(...intersectedOldHistory.toJS());

    const newHistory = ids.slice(0, 1000);
...
const addToMentions =
  (composeId: string, accountId: string) =>
  (dispatch: AppDispatch, getState: () => RootState) => {
    const state = getState();
