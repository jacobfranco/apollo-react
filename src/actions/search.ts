...
const changeSearch = (value: string) => (dispatch: AppDispatch) => {
  ...
  (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    api(getState)
      ...
      .then(async (response) => {
        ...
        dispatch(
          fetchRelationships(data.accounts.map((item: APIEntity) => item.id))
        );
      })
      .catch((error) => {
...
const setFilter = (filterType: SearchFilter) => (dispatch: AppDispatch) => {
