...
const changePassword =
  (oldPassword: string, newPassword: string, confirmation: string) =>
  (dispatch: AppDispatch, getState: () => RootState) => {
...
const resetPasswordConfirm =
  ...
  (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    return api(getState)
      .post("/api/password_reset/confirm", params)
      .then(() => {
...
const confirmChangedEmail =
  (token: string) => (_dispatch: AppDispatch, getState: () => RootState) =>
    api(getState).get(`/api/email/confirm?confirmation_token=${token}`);
