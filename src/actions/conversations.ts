...
const expandConversations =
  ...
  (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    api(getState)
      .get("/api/conversations", { searchParams: params })
      .then(async (response) => {
        ...
        dispatch(
          importFetchedAccounts(
            data.reduce(
              (aggr: Array<APIEntity>, item: APIEntity) =>
                aggr.concat(item.accounts),
              []
            )
          )
        );
        dispatch(
          importFetchedStatuses(
            data
              .map((item: Record<string, any>) => item.last_status)
              .filter((x?: APIEntity) => !!x)
          )
        );
...
const updateConversations =
  (conversation: APIEntity) => (dispatch: AppDispatch) => {
