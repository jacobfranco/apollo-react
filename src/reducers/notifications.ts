...
import { normalizeNotification } from "src/normalizers/notification";
import { validType } from "src/utils/notification";
...
const isValid = (notification: APIEntity) => {
  try {
    ...
    if (
      ["mention", "repost", "like", "poll", "status"].includes(
        notification.type
      ) &&
      !notification.getIn(["status", "id"])
    ) {
      return false;
    }
...
const countFuture = (
  notifications: ImmutableOrderedMap<string, NotificationRecord>,
...
const importNotification = (state: State, notification: APIEntity) => {
  ...
  return state.update("items", (map) => {
    ...
    return map
      .set(notification.id, fixNotification(notification))
      .sort(comparator);
  });
};
...
const filterNotifications = (state: State, relationship: APIEntity) => {
  return state.update("items", (map) =>
    map.filterNot((item) => item !== null && item.account === relationship.id)
  );
};
...
const filterNotificationIds = (
  ...
  const helper = (list: ImmutableOrderedMap<string, NotificationRecord>) =>
    list.filterNot(
      (item) =>
        item !== null &&
        accountIds.includes(item.account as string) &&
...
const importMarker = (state: State, marker: ImmutableMap<string, any>) => {
  const lastReadId = marker.getIn(["notifications", "last_read_id"], -1) as
    | string
    | -1;
