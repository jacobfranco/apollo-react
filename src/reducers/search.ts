...
import { APIEntity, Space, Tag } from "src/types/entities";
...
const ResultsRecord = ImmutableRecord({
  ...
  spaces: ImmutableOrderedSet<Space>(), // it's a list of maps
    ...
    if (state.value === searchTerm && state.filter === searchType) {
      state.set(
        ...
        ResultsRecord({
          ...
          spaces: ImmutableOrderedSet(results.spaces.map(normalizeSpace)), // it's a list of records
          ...
          spacesHasMore: results.spaces.length >= 20,
          accountsLoaded: true,
          statusesLoaded: true,
          groupsLoaded: true,
          hashtagsLoaded: true,
  ...
  return state.withMutations((state) => {
    if (state.value === searchTerm) {
      ...
      state.updateIn(["results", searchType], (items) => {
        ...
        // Spaces and hashtags are a list of maps. Others are IDs.
        if (searchType === "hashtags") {
          return (items as ImmutableOrderedSet<string>).concat(
            (fromJS(data) as Record<string, any>).map(normalizeTag)
          );
        } else if (searchType === "spaces") {
          return (items as ImmutableOrderedSet<string>).concat(
            (fromJS(data) as Record<string, any>).map(normalizeSpace)
          );
        } else {
