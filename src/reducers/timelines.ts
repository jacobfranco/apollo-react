import {
  ...
  const isRepostOf = (repost: Status, status: Status) => repost.repost === status.id;
  ...
  export default function timelines(state: State = initialState, action: AnyAction) {
    switch (action.type) {
      ...
      case TIMELINE_DELETE:
        return deleteStatus(state, action.id, action.accountId, action.references, action.repostOf);
      case TIMELINE_CLEAR:
