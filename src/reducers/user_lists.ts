import {
  ...
  } from 'src/actions/familiar-followers';
  import {
  ...
  export default function userLists(state = ReducerRecord(), action: AnyAction) {
    switch (action.type) {
      ...
      case REPOSTS_FETCH_SUCCESS:
        return normalizeList(state, ['reposted_by', action.id], action.accounts, action.next);
      case REPOSTS_EXPAND_SUCCESS:
        return appendToList(state, ['reposted_by', action.id], action.accounts, action.next);
