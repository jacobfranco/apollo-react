...
const fixMentionsOrder = (status: ImmutableMap<string, any>) => {
  ...
  const sorted = mentions.sort(
    (a: ImmutableMap<string, any>, _b: ImmutableMap<string, any>) => {
...
const fixQuote = (status: ImmutableMap<string, any>) => {
  return status.withMutations((status) => {
    status.update(
      "quote",
      (quote) => quote || status.getIn(["pleroma", "quote"]) || null
    );
    status.deleteIn(["pleroma", "quote"]);
    status.update(
      "quotes_count",
      (quotes_count) =>
        quotes_count || status.getIn(["pleroma", "quotes_count"], 0)
    );
...
const normalizeEmojis = (status: ImmutableMap<string, any>) => {
  const data = ImmutableList<ImmutableMap<string, any>>(
    status.getIn(["pleroma", "emoji_reactions"]) || status.get("reactions")
  );
