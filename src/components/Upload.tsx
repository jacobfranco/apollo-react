...
import AudioPlaceHolder from "src/assets/images/audio-placeholder.png";
...
const Upload: React.FC<IUpload> = ({
  ...
  const handleOpenModal = () => {
    dispatch(
      openModal("MEDIA", { media: ImmutableList.of(media).toJS(), index: 0 })
    );
  };
    ...
    >
      ...
      >
        {({ scale }) => (
          <div
            className={clsx(
              "relative h-40 w-full overflow-hidden bg-contain bg-center bg-no-repeat",
              { "bg-cover ": isMediaCover }
            )}
            style={{
