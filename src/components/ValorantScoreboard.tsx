...
const ValorantScoreboard: React.FC<ValorantScoreboardProps> = ({
  ...
  return (
    <Link
      to={`/games/valorant/scores/${id}}`}
      className={clsx(
        "cursor-pointer p-4 rounded-xl shadow-lg bg-white dark:bg-accent-700 text-gray-900 dark:text-gray-100 grid grid-rows-[auto_1fr_auto] gap-4 max-w-lg mx-auto"
      )}
    >
      <div className="text-lg font-bold text-danger-500">{matchInfo}</div>
