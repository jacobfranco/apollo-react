...
import { useAppDispatch, usePrevious } from 'src/hooks';
...
const ModalRoot: React.FC<IModalRoot> = ({ children, onCancel, onClose, type }) => {
  ...
  const handleModalOpen = () => {
    ...
    unlistenHistory.current = history.listen(({ state }, action) => {
      if (!(state as any)?.apolloModalKey) {
        onClose();
      } else if (action === 'POP') {
        handleOnClose();
  ...
  const handleModalClose = (type: string) => {
    ...
    if (state && (state as any).apolloModalKey === modalHistoryKey.current) {
      history.goBack();
    }
  };
  ...
  const ensureHistoryBuffer = () => {
    ...
    if (!state || (state as any).apolloModalKey !== modalHistoryKey.current) {
      history.push(pathname, { ...(state as any), apolloModalKey: modalHistoryKey.current });
    }
  };
