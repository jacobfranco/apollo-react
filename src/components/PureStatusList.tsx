...
import PureStatus from "src/components/PureStatus";
...
interface IPureStatusList
  extends Omit<IScrollableList, "onLoadMore" | "children"> {
...
const PureStatusList: React.FC<IPureStatusList> = ({
  ...
  const getCurrentStatusIndex = (id: string, featured: boolean): number => {
    if (featured) {
      ...
      return (
        (statuses?.map((status) => status.id) ?? []).findIndex(
          (key) => key === id
  ...
  const handleLoadOlder = useCallback(
    ...
    [onLoadMore, lastStatusId, statuses.slice(-1)?.[0]?.id]
  );
