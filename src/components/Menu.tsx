...
const MenuList: React.FC<IMenuList> = (props) => {
  ...
  return (
    <MenuPopover
      position={props.position === "left" ? positionDefault : positionRight}
    >
      <MenuItems
        onKeyDown={(event) => event.nativeEvent.stopImmediatePropagation()}
        className={clsx(
          className,
          "shadow-menu rounded-lg bg-white py-1 dark:bg-primary-900"
        )}
        {...filteredProps}
      />
    </MenuPopover>
...
/** Divides menu items. */
const MenuDivider = () => (
  <hr className="mx-2 my-1 border-t-2 border-gray-100 dark:border-gray-800" />
);
